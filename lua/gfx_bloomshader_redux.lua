...
local shaderProgs = {
	...
	[H_BLUR_SHADER_IDX] = {
		definitions = {
			...
			"#define kernelSize " .. math.floor(#configParams.blurKernelWeights / 2) .. "\n",
		...
		fragment = [[
			...
			void main(void) {
				vec4 sampleSum = texture2D(sourceTex, gl_TexCoord[0].st) * weights[kernelSize];

				for (int i = 0; i < kernelSize; i++) {
					#if (horizontalPass == 1)
					samples[i                 ] = texture2D(sourceTex, gl_TexCoord[0].st + vec2(float(-kernelSize + i    ) * invTexSizeX, 0.0));
					samples[i + kernelSize + 1] = texture2D(sourceTex, gl_TexCoord[0].st + vec2(float(              i + 1) * invTexSizeX, 0.0));
					#else
					samples[i                 ] = texture2D(sourceTex, gl_TexCoord[0].st + vec2(0.0, float(-kernelSize + i    ) * invTexSizeY));
					samples[i + kernelSize + 1] = texture2D(sourceTex, gl_TexCoord[0].st + vec2(0.0, float(              i + 1) * invTexSizeY));
					#endif

					sampleSum += (samples[i                 ] * weights[i                  ]);
					sampleSum += (samples[i + kernelSize + 1] * weights[i + kernelSize + 1]);
				}
...
function widget:GetInfo()
	return {
		...
		desc      = "applies bloom effect to objects and terrain",
		date      = "24-10-2013",
		license   = "GPL, v2",
		layer     = 0,
		enabled   = CheckEngineVersion(Game.version),
	}
end
