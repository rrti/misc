...
--   (but can use their own deferred lighting system)
--
...
if (gadgetHandler:IsSyncedCode()) then
	...
	function gadget:GetInfo()
        return {
			...
			desc    = "Lua-scriptable dynamic lighting",
			author  = "",
	...
	function gadget:Initialize()
		local modOptionsTbl = Spring.GetModOptions() or {}
		local maxLightsMult = tonumber(modOptionsTbl.dynamicLighting) or 0

		local maxMapLights = (Spring.GetConfigInt("MaxDynamicMapLights"  ) or 0) * maxLightsMult
		local maxMdlLights = (Spring.GetConfigInt("MaxDynamicModelLights") or 0) * maxLightsMult
